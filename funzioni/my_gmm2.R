my_pgmm2<-function (formula, data, subset, na.action, 
    ...
    if (inherits(formula, "dynformula") || length(Formula(formula))[2L] == 
        ...
        if (!inherits(formula, "dynformula")) {
            ...
            m <- match(c("formula", "lag.form", "diff.form", 
                         "log.form"), names(formula), 0)
    ...
    m <- match(c("formula", "data", "subset", "na.action", "index"), 
               names(mf), 0L)
    mf <- mf[c(1L, m)]
    ...
    if (!balanced) {
        ...
        rownames(data) <- paste(index(data, "id"), index(data, 
                                                         "time"), sep = ".")
    ...
    # return(list(data=data, main.form=main.form))
    yX <- extract.data(data, form=main.form) # eu!
    ...
    W <- lapply(W1, remove_non_binary_columns) # tolgo il trattamento dagli strumenti normali
    # return(W)
    # W <-lapply(W , function(x) x[ ,!grepl("D", colnames(x))])

    yX <- yX1
    if (transformation == "d") 
        A1 <- tcrossprod(diff(diag(1, T - TL1 + 1))) # A1= H in latex
    if (transformation == "ld") 
        A1 <- FSM(T - TL2, "full")
    # return(list(W=W, yX=yX))
    WX <- mapply(function(x, y) crossprod(x, y), W, yX, SIMPLIFY = FALSE)
    # WX<-lapply(yX, function(yX_i) crossprod(W, yX_i))
    Wy <- lapply(WX, function(x) x[, 1L])
    WX <- lapply(WX, function(x) x[, -1L, drop = FALSE])
   
    A1 <- lapply(W, function(x) crossprod(t(crossprod(x, A1)), 
                                          x)) # equivalente alle matrici di W nel codice
   
    A1 <- Reduce("+", A1) # versione pesata sotto
    # A1 <- Reduce("+", Map(function(mat, weight) weight * mat, A1, pesi))
    ...
    # A1 <- A1 * sum(pesi)#  length(W) # sum(pesi)
    A1 <- A1 * length(W)
    WX <- Reduce("+", WX)  # versione pesata sotto
    # WX <- Reduce("+", Map(function(mat, weight) weight * mat, WX, pesi))
    Wy <- Reduce("+", Wy) # versione pesata sotto
    # Wy <- Reduce("+", Map(function(mat, weight) weight * mat, Wy, pesi))
    ...
    residuals <- lapply(yX, function(x) as.vector(x[, 1L] - crossprod(t(x[, 
                                                                          -1L, drop = FALSE]), coefficients)))
    outresid <- lapply(residuals, function(x) outer(x, x))
    A2 <- mapply(function(x, y) crossprod(t(crossprod(x, y)), 
                                          x), W, outresid, SIMPLIFY = FALSE)
    A2 <- Reduce("+", A2) # versione pesata sotto
    # A2 <- Reduce("+", Map(function(mat, weight) weight * mat, A2, pesi))
    ...
    if (model == "twosteps") {
        ...
        residuals <- lapply(yX, function(x) {
            nz <- rownames(x)
            z <- as.vector(x[, 1L] - crossprod(t(x[, -1L, drop = FALSE]), 
                                               coefficients))
            names(z) <- nz
            z
        })
    ...
    args <- list(model = model, effect = effect, transformation = transformation, 
                 namest = namesV)
...
# Function to remove non-binary columns from a matrix
remove_non_binary_columns <- function(mat) {
    binary_columns <- apply(mat, 2, is_binary_column)  # Check binary columns
    mat[, !binary_columns]  # Return only the binary columns
}
