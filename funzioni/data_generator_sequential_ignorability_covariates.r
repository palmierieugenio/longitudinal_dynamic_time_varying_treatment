data_generator_SI_cov <- function(N, times, rho, beta, theta, xi, beta_confounder, effetto, statistic,
    ...
    X_0 <-rnorm(N)  # Covariates at time 0                          # Initial X*
    
    X <- matrix(0, nrow=N, ncol=(times + burn_in+pre_treatment)) # Covariates 
    ...
    for (t in 2:total_time) {
        ...
        y[,t] <- alpha + rho * y[,t-1] + beta * X[,t] + beta_confounder * U[,t-1] + rnorm(N, 0, sigma_y) + alpha_t[t]
