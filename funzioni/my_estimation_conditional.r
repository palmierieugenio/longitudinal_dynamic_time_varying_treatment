my_estimation_all_conditional <- function(datu, start=5,  transformation) {
    
    weigths<-tryCatch({
    weigths<-estimate_weights(datu)
    weigths
    },error = function(e) {NA}
    )
    ...
    eff_y6<- tryCatch({
        fit1 <- my_pgmm(y ~ lag(y, 1)+X +D  | lag(y, 2:99), #lag(X)
    ...
    fit2 <- my_pgmm(y ~ lag(y, 1)+X +D+lag(D)  | lag(y, 2:99),
    ...
    eff_y8<- tryCatch({
    fit3 <- my_pgmm(y ~ lag(y, 1)+X +D+lag(D)  | lag(y, 2:99),
